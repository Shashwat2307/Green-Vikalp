...
export default function IndiaMartIntegrationPage() {
  ...
  const generatedWebhookUrl =
    config.campaignId && config.currentStageId
      ? `${typeof window !== "undefined" ? window.location.origin : ""}/api/integrations/webhooks/pabbly/india-mart/${config.campaignId}/${config.currentStageId}`
      : "";
  ...
  return (
    <div className="space-y-6 max-w-5xl mx-auto">
      <div>
        <h1 className="text-2xl font-semibold text-neutral-900">India Mart Integration (via Pabbly Connect)</h1>
        <p className="mt-1 text-sm text-neutral-500">
          Generate a webhook URL to securely receive leads from India Mart through Pabbly Connect.
        </p>
      </div>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <Card>
          <CardHeader>
            ...
            <CardDescription>Select where incoming India Mart leads should be placed.</CardDescription>
          </CardHeader>
        ...
        <Card className="lg:col-span-2">
          <CardHeader>
            ...
            <CardDescription>How to map your India Mart fields to this CRM</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="prose prose-sm max-w-none text-neutral-600 space-y-4">
              <p>
                In Pabbly Connect, create a new workflow with <strong>India Mart</strong> as the Trigger. Then, add an <strong>API by Pabbly</strong> Action and configure it as follows:
              </p>
              ...
              <p>
                Copy the following JSON structure into the <strong>Data</strong> field in Pabbly Connect. Replace the values (e.g., <code>IndiaMart First Name</code>) with the dynamic tags provided by the India Mart trigger:
              </p>
              
              <div className="bg-neutral-900 text-neutral-100 p-4 rounded-lg font-mono text-xs overflow-x-auto">
                <pre>{`{
  "firstName": "IndiaMart First Name",
  "lastName": "IndiaMart Last Name",
  "email": "IndiaMart Email",
  "mobile": "IndiaMart Mobile",
  "leadType": "BUYER",
  "budgetMin": "IndiaMart Budget Min",
  "budgetMax": "IndiaMart Budget Max"
}`}</pre>
              </div>
